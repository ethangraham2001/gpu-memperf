...
from plot_utils import PlotConfig, plot_with_error_bars_raw
...
def plot_combined(results, plot_dir):
    # Combine L1 (1 block), L2 (108 blocks), DRAM (72 blocks)
    # Keys in results are folder names e.g. "random_access_l1"
    ...
    for k in valid_keys:
        ...
        # Ensure x alignment? For now assume same x
...
def plot_all(results_dir: Path, plot_dir: Path):
    ...
    for name, df in results.items():
        plot_single_with_error_bars(name, df, plot_dir)
